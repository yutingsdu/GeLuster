...
#include "p_utility.h"
using namespace std;
ofstream out_my_cluster;
ofstream out_my_cluster2;
...
void get_cluster3(map<string,vector<string> >& seudoref_alignedreads_map)
{
	...
	*/
	...
	{
	    //cout<<seudo_ref[i]<<" * ";
	    out_my_cluster2<<"@"<<seudo_ref[i]<<" ,gene_cluster_"<<idx<<",transcript_cluster_"<<idx<<'\n';
	    /*
...
void get_self_alignment_info(char* file)
{
    ...
    while(getline(in,s))
    {
        istr.str(s);
	string r1,r2;
        istr>>r1>>r2>>temp>>start>>end>>flag>>score;
	istr.clear();
	if( flag != "primary"){
	    //istr.clear();
	    //continue;
...
int main(int argc,char* argv[])
{
    if(argc == 1) {
        cout<<"-"<<endl;
        cout<<"This is a simple program to get the post cluster information from post_read_alignment.info(id chr strand start_pos end_pos flag)."<<endl;
        cout<<"./exe Nx.post.alignment.info"<<endl;
        cout<<"-"<<endl;
	return 0;
    }
    /*
    ...
    */
    get_self_alignment_info(argv[1]);
    Nx = "post";
    string file1 = Nx+".myCluster.tsv";
    string file2 = Nx+".myCluster_multi.tsv";
    string file3 = Nx+".singleton.fasta";
    out_my_cluster.open(file1.c_str());
    out_my_cluster2.open(file2.c_str());
    out_singleton.open(file3.c_str());
